...
const badgeStyles: Record<InvestmentCategory, string> = {
  ...
  ESOPs: "bg-purple-100 text-purple-700",
  "Startup Investments": "bg-orange-100 text-orange-700",
  "Other Investments": "bg-slate-100 text-slate-700",
...
export function InvestmentCategoryBadge({ category }: InvestmentCategoryBadgeProps) {
  return <span className={cn("inline-flex rounded-full px-2.5 py-1 text-xs font-medium", badgeStyles[category])}>{getInvestmentCategoryMeta(category).displayName}</span>;
}