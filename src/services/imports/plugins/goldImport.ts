...
import type { GoldHoldingInsert, GoldHoldingType } from "@/types/goldHolding";

const holdingTypes: GoldHoldingType[] = ["Physical Gold", "Gold ETF", "Sovereign Gold Bond", "Digital Gold"];

interface GoldImportPayload {
  id?: string;
  values: GoldHoldingInsert;
}
...
export const goldImportPlugin: ImportModulePlugin<GoldImportPayload> = {
  moduleId: "gold",
  displayName: "Gold",
  supportedSheets: ["Gold"],
  async validateRows(sheetName, rows) {
    const issues: ImportIssue[] = [];
    const records: Array<ImportValidatedRecord<GoldImportPayload>> = [];
    const existing = await getGoldHoldings();
    ...
    rows.forEach((rawRow, index) => {
      ...
      const values: GoldHoldingInsert = {
    ...
    return {
      ...
      records,
      issues,
    } as ImportValidationResult<GoldImportPayload>;
  },
    ...
    for (const record of records) {
      try {
        if (record.action === "update" && record.payload.id) {
          await updateGoldHolding({ id: record.payload.id, ...record.payload.values });
          updated += 1;
        } else {
          await createGoldHolding(record.payload.values);
          inserted += 1;
        }
      } catch (error) {
        failed += 1;
        issues.push(
          issue({
            ...
            message: error instanceof Error ? error.message : "Unable to import gold row.",
          }),
        );
      }
    }
