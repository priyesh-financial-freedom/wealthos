...
import { createGoldHolding, deleteGoldHolding, getGoldHoldings, updateGoldHolding } from "@/services/goldHoldings";
import type { GoldHolding, GoldHoldingInsert, GoldHoldingType } from "@/types/goldHolding";

const defaultValues: GoldHoldingInsert = {
...
export default function GoldPage() {
  const [rows, setRows] = useState<GoldHolding[]>([]);
  const [loading, setLoading] = useState(true);
  const [query, setQuery] = useState("");
  const [typeFilter, setTypeFilter] = useState<"all" | GoldHoldingType>("all");
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editing, setEditing] = useState<GoldHolding | null>(null);
  const [formValues, setFormValues] = useState<GoldHoldingInsert>(defaultValues);
  ...
  async function refresh() {
    try {
      setLoading(true);
      const data = await getGoldHoldings();
      setRows(data);
      setError(null);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Unable to load gold holdings");
    } finally {
      setLoading(false);
    }
  }
  ...
  function openEditDialog(item: GoldHolding) {
    setEditing(item);
    setFormValues({
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (editing) {
        await updateGoldHolding({ id: editing.id, ...formValues });
        setNotice("Gold holding updated successfully.");
      } else {
        await createGoldHolding(formValues);
        setNotice("Gold holding created successfully.");
      }
      setDialogOpen(false);
      await refresh();
      window.dispatchEvent(new Event("wealthos:finance-data-updated"));
    } catch (err) {
      setError(err instanceof Error ? err.message : "Unable to save gold holding");
    } finally {
      setSubmitting(false);
    }
  }

  async function handleDelete(item: GoldHolding) {
    const confirmed = window.confirm(`Delete ${item.description}?`);
    if (!confirmed) return;

    try {
      await deleteGoldHolding(item.id);
      setNotice("Gold holding deleted successfully.");
      await refresh();
      window.dispatchEvent(new Event("wealthos:finance-data-updated"));
    } catch (err) {
      setError(err instanceof Error ? err.message : "Unable to delete gold holding");
    }
  }
    ...
    <AppLayout>
      <PageContainer>
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <PageHeader
            title="Gold"
            description="Track physical gold, Gold ETF, Sovereign Gold Bonds, and digital gold in one module."
          />
          <Button onClick={openCreateDialog}>Add Gold Holding</Button>
        </div>
        ...
        <DashboardCard>
          <div className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
            <div className="space-y-2">
              <Label htmlFor="gold-search">Search</Label>
              <Input id="gold-search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search by type, description, owner, custodian" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="gold-type">Type</Label>
              <select id="gold-type" className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm" value={typeFilter} onChange={(event) => setTypeFilter(event.target.value as "all" | GoldHoldingType)}>
        ...
        <DashboardCard>
          {loading ? (
            <LoadingSpinner label="Loading gold holdings..." />
          ) : filteredRows.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-6 py-12 text-center text-sm text-slate-600">No gold holdings yet.</div>
          ) : (
          ...
          <DialogContent className="max-w-3xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid gap-4 md:grid-cols-2">
                <FieldSelect label="Holding Type" value={formValues.holding_type} options={["Physical Gold", "Gold ETF", "Sovereign Gold Bond", "Digital Gold"]} onChange={(value) => setFormValues((current) => ({ ...current, holding_type: value as GoldHoldingType }))} />
                ...
                <div className="space-y-2 md:col-span-2">
                  <Label htmlFor="gold-notes">Notes</Label>
                  <Textarea id="gold-notes" rows={3} value={formValues.notes ?? ""} onChange={(event) => setFormValues((current) => ({ ...current, notes: event.target.value }))} />
                </div>
              </div>
...
function FieldText({ label, value, onChange, type = "text" }: { label: string; value: string; onChange: (value: string) => void; type?: string }) {
  const id = `gold-${label.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`;
  return (
    <div className="space-y-2">
